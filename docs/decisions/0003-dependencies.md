...
代理不属于本仓库的依赖图。当前独立部署使用 FastAPI 与 `httpx`，但调用方只依赖
[代理契约](../proxy-contract.md)和 HTTPS 地址；服务可以在不改变 npm 包的前提下替换实现语言。

| 用途 | 选择 | 理由 |
| --- | --- | --- |
| Web 框架 | FastAPI | 服务已在用 |
| HTTP 客户端 | `httpx` | 异步，超时与重定向限制可配 |
| 测试 | `unittest` | 覆盖转发语义、白名单与认证，不引入额外测试框架 |
...
**独立代理服务参考实现**（2026-08-11，读取外部服务源码确认）。其依赖包括：
...
- 微信调用集中在 Node 客户端一处，错误分类和配额策略不受第三方封装限制。

### 负面影响

- 渲染质量的下限由我们自己保证，社区主题多年打磨出的排版细节需要逐步补齐。
- 完整部署跨 Node 与代理服务两套运行时，但父仓库只维护 Node 依赖和测试。
